-- UK Bus profile
-- TODO: add support for busway=opposite_lane https://wiki.openstreetmap.org/wiki/Bus_lanes
api_version = 4
...
function setup()
  return {
    properties = {
      max_speed_for_map_matching      = 112/3.6,
    ...
    -- I had to reduce this size, otherwise it didn't follow the actual bus routes in some roads.
    vehicle_height = 3.5, -- in meters; 4.38m is the approx. height of a double decker.
    vehicle_width = 2.1, -- in meters; 2.9m is the width of a double decker. Ways with narrow tag are considered narrower than 2.2m.
...
-- handle oneways tags
function WayHandlers.handle_bus_oneway(way,result,data,profile)
